...
export function regionalRanking(game,year=Math.floor((game.day-1)/SEASON.days)+1){
  const rows=new Map();
  const row=x=>{
    if(!rows.has(x.id))rows.set(x.id,{id:x.id,name:x.name,agencyName:x.agencyName,rating:R.base,
      points:0,matches:0,wins:0,cityMatches:0,form:[],lastDay:0,role:x.definition.regionalRole||'pro'});
    return rows.get(x.id);
  };
  const events=(game.competitions?.events||[]).filter(e=>e.season===year&&e.status==='completed'&&e.ranking).sort((a,b)=>a.date-b.date||a.id.localeCompare(b.id));
  ...
  const current=new Map([...game.league.agencies.flatMap(a=>a.roster),...(game.personnel?.freeAgents||[])].map(t=>[t.id,t]));
