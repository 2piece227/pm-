...
import { youthStats, youthCandidates } from '../data/youth-candidates.js';
...
export function agreeYouth(game,candidate,offer){
  candidate=youthCandidates(game).find(c=>c.id===candidate?.id);
  if(!candidate)throw new Error('현재 스카우트 명단에 없는 후보입니다. 목록을 새로 확인하세요.');
  const agency=playerAgency(game);
  if(game.pendingStarter||rosterLock(game)||game.league.agencies.some(a=>a.roster.some(t=>t.id===`y-${candidate.id}`)))throw new Error('현재 이 후보와 추가 계약할 수 없습니다.');
