...
import { fieldState, movePP } from '../engine/field-state.js';
...
export function moveTile(move, index, mon=null) {
  const m=Dex.moves.get(move||''),pp=movePP(mon,move);
  return `<span class="move-type" data-type="${m.type||'Normal'}">${TYPE_KO[m.type]||'빈 칸'}</span><b>${move?esc(M(move)):'새 기술 등록'}</b><small>${move?`PP ${pp.current}/${pp.max} · ${m.category==='Physical'?'물리':m.category==='Special'?'특수':'변화'}`:`기술 ${index+1}`}</small>`;
}
...
export function openPokemonPanel(game, mon, onChange, initialTab = 'moves', initialSlot = 0) {
  ...
  let tab=initialTab,slot=Math.min(initialSlot,mon.moves.length,3),selected=mon.moves[Math.min(initialSlot,mon.moves.length,3)]||null,query='',notice='',pending=false;
  const close=()=>{if(pending)return;dialog.close();dialog.remove();};
  const draw=()=>{
    const species=Dex.species.get(mon.species), health=fieldState(mon),locked=!!mon.training;
    ...
    const equipped=mon.moves.includes(selected),pp=movePP(mon,selected);
    dialog.innerHTML=`<header class="pokemon-heading">${monImage(mon.species)}<div><small>POKÉMON / Lv.${mon.level}</small><h2>${esc(K(mon.species))}</h2><p>${species.types.map(t=>TYPE_KO[t]).join(' / ')} · HP ${health.hp}/${health.maxhp}</p></div><button class="ghost close-pokemon" aria-label="포켓몬 관리 닫기">✕</button></header>
      ...
      ${locked?'<p class="panel-notice">장기 훈련 중입니다. 정보는 볼 수 있지만 변경은 훈련 완료 또는 취소 후 가능합니다.</p>':''}
      ${tab==='moves'?`<p class="muted">바꿀 기술 칸을 고른 다음 새 기술을 선택하세요. 변경 1회 ${MANAGEMENT.movePrice}.</p>
      <div class="move-slots">${Array.from({length:4},(_,i)=>`<button class="move-slot ${slot===i?'selected':''}" data-slot="${i}" ${i>mon.moves.length?'disabled':''}>${moveTile(mon.moves[i],i,mon)}</button>`).join('')}</div>
      <div class="move-library"><section><label>배운 기술 검색<input id="move-search" value="${esc(query)}" placeholder="기술 이름"></label><div class="move-candidates">${moves.map(m=>`<button class="move-choice ${selected===m?'selected':''}" data-move="${esc(m)}" aria-pressed="${selected===m}"><b>${esc(M(m))}</b><span>${TYPE_KO[Dex.moves.get(m).type]} ${mon.moves.includes(m)?'· 장착 중':''}</span></button>`).join('')||'<p>검색 결과가 없습니다.</p>'}</div></section>
      <aside class="move-detail">${selectedData?`<span class="move-type" data-type="${selectedData.type}">${TYPE_KO[selectedData.type]}</span><h3>${esc(M(selected))}</h3><dl><dt>위력</dt><dd>${selectedData.basePower||'—'}</dd><dt>명중</dt><dd>${selectedData.accuracy===true?'필중':selectedData.accuracy}</dd><dt>PP</dt><dd>${pp.current} / ${pp.max}</dd><dt>분류</dt><dd>${selectedData.category==='Physical'?'물리':selectedData.category==='Special'?'특수':'변화'}</dd></dl><p class="muted">${esc(selectedData.shortDesc||'')}</p><p>${equipped?'현재 파티에 장착된 기술입니다.':`${mon.moves[slot]?esc(M(mon.moves[slot])):'빈 칸'} → ${esc(M(selected))}`}</p><button id="confirm-move" ${locked||equipped||agencyOf(game).funds<MANAGEMENT.movePrice?'disabled':''}>${locked?'훈련 중 · 변경 불가':equipped?'이미 장착한 기술':agencyOf(game).funds<MANAGEMENT.movePrice?'자금 부족':`${MANAGEMENT.movePrice} 지불 · 기술 변경`}</button>`:'<p class="muted">기술을 선택하면 위력·명중·PP와 변경 내용을 확인할 수 있습니다.</p>'}</aside></div>`:''}
      ${tab==='ability'?`<h3>현재 특성 · ${esc(A(mon.ability))}</h3><p class="muted">특성 변경은 ${MANAGEMENT.abilityPrice}가 필요합니다.</p><div class="ability-options">${Object.entries(species.abilities).map(([key,value])=>`<button class="ability-choice ${value===mon.ability?'selected':''}" data-ability="${esc(value)}" ${locked||value===mon.ability?'disabled':''}><b>${esc(A(value))}</b><small>${key==='H'?'숨겨진 특성':'일반 특성'}${value===mon.ability?' · 적용 중':''}</small></button>`).join('')}</div><div id="ability-confirm"></div>`:''}
      ${tab==='evolution'?`<p class="muted">현재 레벨 ${mon.level} · 조건을 만족하면 진화할 수 있습니다.</p>${evolutionOptions(mon).map(e=>`<div class="evolution-option">${monImage(e.species)}<div><h3>${esc(K(e.species))}</h3><p>${esc(e.reason)}</p><button data-evolve="${esc(e.species)}" ${e.ready?'':'disabled'}>${e.ready?'진화하기':locked?'훈련 중 · 진화 대기':'조건 미충족'}</button></div></div>`).join('')||'<p>최종 진화 형태입니다.</p>'}${species.evos.length?`<label class="auto-evolve"><input type="checkbox" id="auto-evolve" ${locked?'disabled':''} ${mon.autoEvolve?'checked':''}> 레벨 조건을 만족하면 경험치 획득 후 자동 진화</label>`:''}<p class="muted">돌·교환·친밀도 등 특수 조건은 자동 진화 대상이 아닙니다.</p>`:''}
    ...
    dialog.querySelectorAll('[data-slot]').forEach(b=>b.onclick=()=>{slot=Number(b.dataset.slot);selected=mon.moves[slot]||null;draw();});
