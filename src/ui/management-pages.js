...
export function renderGyms(game){
...
export function wireManagementPages(root,game,refresh,watch){
  ...
  root.querySelectorAll('[data-gym]').forEach(b=>b.onclick=()=>{const id=root.querySelector('#gym-trainer')?.value;if(!id)return;game.actions[id]=`gym:${b.dataset.gym}`;refresh();notice('체육관 도전을 배정했습니다. 계속을 누르면 준비 후 관전을 시작합니다.');});
