 ...
 * 스프라이트 레이어 — 표시 전용.
 *
 * 실제 도트는 포켓몬쇼다운 스프라이트를 쓴다. SPEC §2의 "HGSS풍 고정 도트"에 맞춰
 * gen4(DP/HGSS 세대) 세트를 요청하고, 그 세대에 없는 포켓몬은 @pkmn/img가
 * 자동으로 상위 세대 도트로 떨어뜨린다 (예: 드래펄트 → gen5).
 *
 * SPEC §10 — 엔진(@pkmn/sim)은 MIT라 문제없지만 스프라이트는 별개 사안이다.
 * 그래서 이미지 출처를 이 파일 한 곳에 가둬 둔다. 오리지널 IP로 껍데기를 갈아끼울 때
 * spriteUrl() 하나만 바꾸면 나머지 코드는 손댈 필요가 없다.
 */
import { Sprites } from '@pkmn/img';

const SPRITE_GEN = 'gen4';
