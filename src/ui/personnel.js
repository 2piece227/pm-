...
function negotiate(game,id,renew,refresh){
  ...
  dialog.querySelector('#personnel-propose').onclick=()=>{
    ...
    const line=document.createElement('p');line.textContent=`${t.name}: ${r.msg}`;dialog.querySelector('[role=log]').append(line);
    if(r.ok){dialog.querySelector('#personnel-propose').disabled=true;dialog.querySelectorAll('input').forEach(i=>i.disabled=true);refresh();}
  };
}
