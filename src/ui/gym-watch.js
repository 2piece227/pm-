...
export function watchGymMatch(match) {
  return new Promise(resolve=>{
    ...
    let closed=false;
    const close=()=>{
      if(closed)return;closed=true;stopPlayback();sfx.stopAll();overlay.hidden=true;app.inert=false;
    ...
    $('watch-pause').onclick=()=>{$('watch-pause').textContent=setPaused(!isPaused())?'▶ 재개':'⏸ 일시정지';};
    ...
    playBattleLog(match.log,{p1:{name:safe(match.trainerName),agency:safe(match.agencyName)},p2:{name:match.gymName,agency:'체육관 관장'}})
      .then(()=>{if(!closed){$('watch-title').textContent=`${match.trainerName} · ${match.won?'관장전 승리':'관장전 패배'}${match.firstWin?' · 배지 획득':''}`;$('watch-close').textContent='결과 확인 · 닫기';}})
      .catch(()=>{if(!closed){$('watch-title').textContent='관전 표시를 완료하지 못했습니다. 경기 결과는 저장되어 있습니다.';$('watch-close').textContent='결과 확인 · 닫기';}});
  });
}
